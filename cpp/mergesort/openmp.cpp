...
int main(int argc, char* argv[]) {
    ...
    std::cerr << "OpenMP support is not available in this build\n";
    return 1;
#else
    ...
    if (dtype == "u32") {
        ...
        Timer timer;
        seminar::mergesort_openmp(data, cutoff, threads);
        ...
        if (actual_sum != expected_sum) {
            std::cerr << "checksum mismatch: " << actual_sum << " != " << expected_sum << "\n";
            return 1;
        }
        emit_csv("mergesort", input, "cpp_openmp", threads, run_id, elapsed, actual_sum);
